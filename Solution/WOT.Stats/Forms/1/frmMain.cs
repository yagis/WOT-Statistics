...
namespace WOT.Stats
{
    ...
    public partial class frmMain : DevExpress.XtraEditors.XtraForm
    {
        ...
        public frmMain()
        {
            ...
            DevExpress.XtraSplashScreen.SplashScreenManager.Default.SendCommand(SplashS.SplashScreenCommand.SetCurrentAction, "Initializing Components");
            ...
            using (PlayerListing players = new PlayerListing(_message))
            {
                if (players.Count() > 0)
                {
                    ...
                    {
                        DevExpress.XtraSplashScreen.SplashScreenManager.Default.SendCommand(SplashS.SplashScreenCommand.SetCurrentAction, "Initializing " + player.Value.PlayerID);
                        ...
                        if (File.Exists(player.Value.WatchFile))
                        {
                            ...
                            DevExpress.XtraSplashScreen.SplashScreenManager.Default.SendCommand(SplashS.SplashScreenCommand.SetCurrentAction, "Initializing Monitor");

                            if (UserSettings.StartMonOnStartUp == true)
                            {
                                dm.StartDossierWatch();
                                barStaticItem1.Glyph = WOT.Stats.Properties.Resources.bullet_square_green;
                                barStaticItem1.Caption = "Monitor Status : Running";
                            }
                            else
                            {
                                barStaticItem1.Glyph = WOT.Stats.Properties.Resources.bullet_square_red;
            ...
            Console.WriteLine("frmMain DONE");
        }
        ...
        private void BuildGraphForm(string playerName)
        {
            ...
            {
                try { splashScreenManagerWaitForm.CloseWaitForm(); }
                catch { }
                finally
                {
                    _chart.Series.Clear();
        ...
        private void frmMain_Shown(object sender, EventArgs e)
        {
            if (UserSettings.NewVersionNotify)
            {
                ...
                _message.Add("Info : New update available. (" + UserSettings.NewAppVersion + "). Please visit www.vbaddict.net/wotstatistics");
                if (DevExpress.XtraEditors.XtraMessageBox.Show(Translations.TranslationGet("STR_VERSIONNOTIFY", "DE", "New update available (" + UserSettings.NewAppVersion + ")." + Environment.NewLine + "Would you like to open the web page www.wotstatistics.net").Replace("{version}", UserSettings.NewAppVersion), "WOT Statistics", MessageBoxButtons.YesNo, MessageBoxIcon.Information) == System.Windows.Forms.DialogResult.Yes)
                {
